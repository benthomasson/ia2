...
def linear_interpolation(
    ...
    return (
        truncated_index_weight * array[truncated_index]
        + next_index_weight * array[next_index]
    )
...
def build_samples(
    sample_table: Dict[str, npt.NDArray[np.floating]],
    waveform: Callable,
    ...
    for note, f in NOTES.items():
        ...
        output = np.zeros((sample_rate * duration,), dtype=np.float32)

        index = 0.0
        index_increment = f * wavetable_length / sample_rate

        for n in range(output.shape[0]):
            output[n] = linear_interpolation(wavetable, index)
